...
<?php
	...
	if (isset($_POST['submit'])) {
		...
		// query to add a new manager
		$insertEmployee = "INSERT INTO `employees` (FirstName, LastName, Phone, Email, DepartmentID, AccountType, Auth)
							VALUES ('$firstName', '$lastName', '$phoneNumber', '$email', '$departmentID', '2', '$passHash');";
							
		// query to check that the regCode is correct
		$getRegCode = "SELECT `RegCode`
						FROM `departments`
						WHERE `DepartmentID` = $departmentID
						LIMIT 1";
						
		// Checks that the user has the correct authorization to create a manager account
		$resultHash = mysqli_query($con, $getRegCode);
	
		if ($resultHash) {
			
			$row = mysqli_fetch_row($resultHash);
			$databaseRegHash = $row[0];
			
		}
		
		// if the two hash variables are equal, it will allow the user to create the account
		if (hash_equals($databaseRegHash, crypt($regCode, 'dsGdpl%d!'))) {
			...
			// gives an error message if the user did not give a valid regCode
			echo '</br></br>';
			echo '<center><p style=color: red;">You do not have authorization to create a new manager account.</p><center>';
			echo '<center><p style=color: red;">Please <a href="addNewEmployee.php">click here</a> to create an employee account.</p><center>';
...
<html>
	<head>
		<title>Add New Manager | Enterprise Career Portal</title>
		<meta charset="UTF-8" />
	...
	<div class="main">
		...
		Registering as an employee? <a href="addNewEmployee.php">Click Here</a>!
		
		<!-- Asks for employee details to create a new account.  -->
		<form method="post" action="addNewManager.php">
		<table>
			<tr>
			...
			<tr>
				<td align="right">Enter your registration code: </td>
